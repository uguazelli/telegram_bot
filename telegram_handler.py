...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    print(f"[BOT] Received /start command from user_id={update.effective_user.id}", flush=True)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"[BOT] Received message from user_id={user_id}: '{user_input}'", flush=True)

    try:
        intent = classify_intent(user_input)
        print(f"[BOT] Intent classified as: {intent}", flush=True)

        if intent == "business":
            print(f"[BOT] Running RAG for business query...", flush=True)
            answer = orchestrate_chat(DOC_PATH, user_input)
            print(f"[BOT] RAG answer:\n{answer}")
        else:
            print(f"[BOT] Using memory chat chain...", flush=True)
        ...
        print(f"[ERROR] Exception in handle_message: {e}", flush=True)

    await update.message.reply_text(answer)

def main():
    print("[INIT] Starting Telegram bot...", flush=True)
    ...
    print("[INIT] Bot is polling for messages.", flush=True)
    app.run_polling()
